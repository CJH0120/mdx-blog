...
- ### SASS
  ...
  - 변수, 중첩 규칙, 믹스인, 상속 등 CSS에 없는 추가 기능을 제공하여, 스타일 시트를 더 효율적으로 작성하고 유지보수할 수 있다.

- ### highlight.js

  - 다양한 프로그래밍 언어의 문법을 인식하고, 코드 블록을 색상으로 강조하여 가독성을 높여주는 라이브러리.
...
```ts
const parseMarkdownFile = async (filePath: string): Promise<MDX.Metadata | null> => {
...
**메인 페이지에서 useReadMdx를 호출하여 메타데이터를 가져오고, 각 메타데이터를 ArticleCard 컴포넌트에 넘겨서 렌더링**
...
## 그러면 블로그 게시글에서는 어떻게 데이터를 가져오는걸까?
...
**`_markdown` 디렉토리에서 해당 경로와 일치하는 파일을 찾아 메타데이터와 본문을 반환한다.**
    ...
    <>
      ...
      {!!RecommendData.length && (
        <div className={[styles['recommend-box-wrapper']].join(' ')}>
          {RecommendData.map((v) => Object.keys(v).map((key) => <RecommendBox key={key} recommendItem={v[key]} keyItem={key} />))}
        </div>
      )}
    </>
  );
...
오늘은 블로그에서 데이터를 어떻게 불러오는지, 그리고 이를 위해 어떤 라이브러리를 사용했는지 알아보았습니다. 다음 게시글에서는 아직 설명하지 않은 useReadMdxTags 및 Next.js 제공 함수에 대해 알아 볼 예정이다.

전체 코드는 [깃허브](https://github.com/CJH0120/mdx-blog) 에 나와있으니 **folder structure** 어떻게 되어 있는지 한번 확인해보시고 피드백을 주면 좋겠다.
